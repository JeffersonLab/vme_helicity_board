...
int WriteHelBoard(long quantity, long value)
{
  ...
  printf("vmeBusToLocalAdrs result = %d \n",res);
  if (res != 0) {
       printf("-------- HelBoard: ERROR:  vmeBusToLocalAdrs address %x\n",laddr);
       vmeBusUnlock();
  ...
  vmeWrite16(&hBd->registers[quantity],vwrite);
  printf("about to unlock \n");
  vmeBusUnlock();
  printf("about to close windows \n");
  vmeCloseDefaultWindows();
  printf("all done \n");
  return OK;
 }
...
int ReadHelBoard(long quantity)
{
  ...
  hBd = (struct HELBOARD *) laddr;
  board_register = vmeRead16(&hBd->registers[quantity]); 
